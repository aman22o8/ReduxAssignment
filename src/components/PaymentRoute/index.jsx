...
const PaymentRoute = () => {
    ...
    const cartSummaryState=useSelector((state)=> state.addtoCart)
    ...
    console.log("cart Summary",cartSummaryState)
  return (
    <div className='flex justify-between mt-[30px] mx-[20px]'>
        ...
        <div className=' bg-[#ECFCCB] rounded-[10px] p-[15px] mr-[5vw] w-[38%] flex flex-col items-center'>
            ...
            <ul className='list-none'>{
                cartSummaryState.listItemAdded.map((each)=>{
                    return (
                        <li className='p-[5px] border-2 rounded-xl mb-2 border-zinc-700' key={each.id}>
                            <img src={each.imageUrl} className=' rounded-[5px] mix-blend-multiply h-[35px] w-[30px]' alt="cartImage" />
                ...
                <h1 className=' text-2xl my-[10px] font-[700] font-[Roboto] '>{`Total : $ ${cartSummaryState.totalamount}`}</h1>
